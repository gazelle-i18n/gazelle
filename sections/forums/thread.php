...
<?
			...
			$StaffNames = $Cache->get_value('staff_names');
			if(!$StaffNames) {
				$DB->query("SELECT um.Username AS Username
					FROM users_main AS um 
						JOIN permissions AS p ON um.PermissionID=p.ID 
					WHERE p.DisplayStaff='1'");
				$StaffNames = array_values($DB->collect('Username'));
...
if (check_perms('site_moderate_forums') && $ThreadInfo['Posts'] > 1){ ?>				- <a href="#post<?=$PostID?>" onclick="Delete('<?=$PostID?>');">[Delete]</a> <? } ?>
